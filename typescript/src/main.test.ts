...
describe('app elements', () => {
  ...
  it('should match page title & header with app name', () => {
    const appName = 'Typescript Todo List';
    const titleElement = document.querySelector<HTMLTitleElement>('title');
    const headingElement = document.querySelector<HTMLHeadingElement>('h1');
...
describe('localStorage functions', () => {
  ...
  const localStorageMock = (function () {
    ...
    return {
      getItem(key: keyof localStorageStore) {
        return store[key];
      },

      setItem(key: keyof localStorageStore, value: localStorageStore[keyof localStorageStore]) {
        store[key] = value;
      },
      ...
      removeItem(key: keyof localStorageStore) {
        delete store[key];
      },
